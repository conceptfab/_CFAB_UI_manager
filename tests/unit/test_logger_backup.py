...
from utils.logger import AsyncLogger, AppLogger, initialize_logger, get_logger
...
class TestAsyncLogger(unittest.TestCase):
    ...
    def test_log_exception(self):
        ...
        # Check stats
        stats = self.async_logger.get_performance_stats()
        self.assertEqual(stats["logs_processed"], 1)
        self.assertEqual(stats["errors"], 1)
        self.assertEqual(stats["log_levels_count"]["error"], 1)
    ...
    def test_performance_stats(self):
        ...
        # Check stats
        stats = self.async_logger.get_performance_stats()
        self.assertEqual(stats["logs_processed"], 10)
        self.assertEqual(stats["log_levels_count"]["info"], 5)
        self.assertEqual(stats["log_levels_count"]["warning"], 3)
        self.assertEqual(stats["log_levels_count"]["error"], 2)
        self.assertEqual(stats["errors"], 2)
        self.assertEqual(stats["warnings"], 3)
    ...
    def test_stop(self):
        ...
        stats = self.async_logger.get_performance_stats()
        self.assertEqual(stats["logs_processed"], 1)  # Still only 1 processed
...
class TestAppLogger(unittest.TestCase):
    ...
    def test_log_methods(self):
        ...
        self.assertEqual(stats["logs_processed"], 6)  # 5 + setup message
        
        # Check that files were created
        self.assertTrue(os.path.exists(os.path.join(self.test_log_dir, "TestLogger.log")))
        self.assertTrue(os.path.exists(os.path.join(self.test_log_dir, "TestLogger_errors.log")))
    ...
    def test_log_level_change(self):
        ...
        # Check stats
        stats = self.app_logger.get_performance_stats()
        # 1 setup + 1 debug + 1 set_level info + 1 info after = 4
        self.assertEqual(stats["logs_processed"], 4)
