...
from PyQt6.QtWidgets import QProgressBar, QSplashScreen, QLabel
...
class EnhancedSplashScreen(QSplashScreen):
    ...
    def __init__(
        ...
        if pixmap.isNull():
            ...
            logger.warning(f"Could not load splash image from {image_path}, using default")
        else:
    ...
    def start_with_tasks(self, tasks: List[str]) -> None:
        ...
        logger.info(f"Splash screen started with {self.total_tasks} tasks")
    ...
    def update_progress(self, task_name: str, completed: bool = True) -> None:
        ...
        # Update message
        if completed:
            progress_percentage = int((self.completed_tasks / max(1, self.total_tasks)) * 100)
            ...
            logger.debug(f"Task completed: {task_name} ({self.completed_tasks}/{self.total_tasks})")
        else:
...
def create_optimized_splash(
    ...
    logger.info(f"Created optimized splash screen with {len(startup_tasks)} tasks")
...
if __name__ == "__main__":
    ...
    splash, tracker = create_optimized_splash(
        image_path="resources/img/splash.jpg",
        startup_tasks=tasks
    )
