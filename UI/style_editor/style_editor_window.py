...
from PyQt6.QtCore import QIODevice, QSaveFile, Qt  # Dodano QIODevice
...
class StyleEditorWindow(QMainWindow):
    def __init__(self):
        ...
        self.save_as_button.clicked.connect(
            lambda: self.save_style_to_file(save_as=True)
        )
        ...
        self.qss_text_edit.setPlaceholderText(
            "Wpisz tutaj swój QSS lub wczytaj z pliku..."
        )
        self.qss_text_edit.setAcceptRichText(False)
        self.qss_text_edit.setStyleSheet(
        ...
        if os.path.exists(default_style_path):
            ...
            logger.info(f"Załadowano domyślny styl: {default_style_path}")
        else:
            logger.warning(f"Domyślny plik stylów nie znaleziony: {default_style_path}")
            self.apply_style()
    ...
    def load_style_from_file(self, path=None):
        if not path:
            path, _ = QFileDialog.getOpenFileName(
                self,
                "Wczytaj plik QSS",
                self.current_style_path or "",
                "Pliki QSS (*.qss);;Wszystkie pliki (*)",
            )
        if path:
            try:
    ...
    def save_style_to_file(self, save_as=False):
        ...
        if save_as or not path:
            ...
            default_save_dir = (
                os.path.dirname(self.current_style_path)
            ...
            path, _ = QFileDialog.getSaveFileName(
                self,
                "Zapisz plik QSS",
                self.current_style_path or default_filename,
                "Pliki QSS (*.qss);;Wszystkie pliki (*)",
            )
            ...
            try:
                ...
                if save_file.open(
                    ...
                    if save_file.write(text_to_save.encode("utf-8")) == -1:
                        raise IOError(
                            f"Błąd zapisu do QSaveFile (write zwrócił -1): {save_file.errorString()}"
                        )

                    if not save_file.commit():
                        raise IOError(
                            f"Błąd commit QSaveFile: {save_file.errorString()}"
                        )

                    self.current_style_path = path
                    self.setWindowTitle(
                        f"Edytor Stylów PyQt6 - {os.path.basename(path)}"
                    )
                    QMessageBox.information(self, "Sukces", f"Zapisano styl w: {path}")
                else:
                    raise IOError(
                        f"Nie można otworzyć QSaveFile do zapisu: {save_file.errorString()}"
                    )
            except Exception as e:
